...
```C++
if (results.value == BUTTON_0) { pos = 0; }
if (results.value == BUTTON_1) { pos = 20; }
if (results.value == BUTTON_2) { pos = 40; }
   ... （あと7個）
```
...
> **`#define` って何？**
> `#define BUTTON_1 0xFFA25D` は「`BUTTON_1` という名前で `0xFFA25D` を表す」という意味。
> `0xFFA25D` のままだと何のことか分かりませんが、`BUTTON_1` なら一目で分かりますね。
