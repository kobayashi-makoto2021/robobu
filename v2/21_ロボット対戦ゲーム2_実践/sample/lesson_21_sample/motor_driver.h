 ...
 * ⚠ ピン配置について（重要）
 *
 *   右モーターのPWM（ENA）は、ふだんは D3 を使う。
 *   旧教材では D9 だったが、D9 はサーボモーター用に空けておく。
 *   （実践17の迷路チャレンジ2でサーボを使うため）
 *
 *   ただし、赤外線（IRremote）を使うレッスンだけは D9 を使う。
 *   IRremoteはArduino UNOの「タイマー2」という部品を使うが、
 *   D3の速さの調節（analogWrite）も同じタイマー2を使うため、
 *   両方をいっしょには使えない。
 *
 *   → 実践14・20・21 では、#include より前に次の1行を書くこと。
 *        #define USE_IR_ROBOT
 *        #include "motor_driver.h"
 *      配線も ENA を D9 につなぎかえる。
 * ------------------------------------------------------------------
 */
...
// ===== ピンの設定 =====
#ifdef USE_IR_ROBOT
  #define speedPinR        9   // 右モーターのPWM（ENA）赤外線を使うレッスンだけD9
#else
...
// 下がりながら左へ（右タイヤだけを後ろに回す）
// タブレットのアプリの「X」ボタン用。実践19・20・21で使う
void back_Left(int speed = 200, int time = 0)
{
  digitalWrite(RightMotorDirPin1, LOW);
  digitalWrite(RightMotorDirPin2, HIGH);
  digitalWrite(LeftMotorDirPin1, HIGH);
...
// 下がりながら右へ（左タイヤだけを後ろに回す）
// タブレットのアプリの「Y」ボタン用。実践19・20・21で使う
void back_Right(int speed = 200, int time = 0)
{
  digitalWrite(RightMotorDirPin1, HIGH);
  digitalWrite(RightMotorDirPin2, LOW);
  digitalWrite(LeftMotorDirPin1, LOW);
