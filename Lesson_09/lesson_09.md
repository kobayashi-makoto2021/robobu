...
#### ソフトウェアの準備をしよう
- [ ] 新しい白紙のスケッチを作る
- [ ] 「（日付4桁）(自分の名前・ローマ字)08-01」という名前でスケッチを保存
...
#### ソフトウェアの準備をしよう
- [ ] 新しい白紙のスケッチを作る
- [ ] 「（日付4桁）(自分の名前・ローマ字)08-02」という名前でスケッチを保存
...
void loop(){
  if (IrReceiver.decode()){  // 赤外線受信機モジュールの受信データ
    ...
    Serial.println(IrReceiver.decodedIRData.numberOfBits);        // ビット数を出力する
    //「OK」ボタンが押されたら、LEDが光る
    if (IrReceiver.decodedIRData.decodedRawData == 0x******) {  // ←【ここを書き換えよう】
      digitalWrite(3, HIGH);
      Serial.print("HIGH\n");
    }
    //「#」ボタンが押されたら、LEDが消える
...
- **ライブラリ**:プログラムを効率的に開発できるようにあらかじめ作ってあるプログラムのこと

### 出来たことをチェックしよう
- [ ] ジャンパー線を使って**赤外線受信モジュール**の配線ができた
- [ ] IRremote**ライブラリ**を追加できた
- [ ] サンプルコードを実行できた
- [ ] 付属のリモコンからの信号を確認できた
- [ ] 他の付属ではないリモコンの信号を確認できた
- [ ] （発展）赤外線受信の原理を説明できた
